...
test.describe('Drive — DocType lists (P1)', () => {
  ...
  test('Drive Settings single loads', async () => {
    const doc = await client.call<{ name: string }>('frappe.client.get', {
      doctype: 'Drive Settings', name: 'Drive Settings',
    });
    expect(doc.name).toBe('Drive Settings');
  });
});
